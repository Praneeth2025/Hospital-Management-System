...
const TodaysAppointment = () => {
    ...
    try {
      ...
      if (response.status === 200) {
        const updatedData = clinicData.map((row) =>
          checkedRows[row.patient_id] ? { ...row, schedule_type: "Completed" } : row
        );
  ...
  const filteredData = clinicData
    .filter((row) => row.schedule_type === "Inside the Cabin")
    .filter((row) =>
          ...
          <table className="min-w-full border divide-y divide-gray-200 shadow-md">
            ...
            <tbody className="bg-white divide-y divide-gray-100">
              {filteredData.map((row) => {
                const rowKey = row.patient_id;
                const isChecked = checkedRows[rowKey] || false;
